...
void runSingleCase(const std::filesystem::path& casePath, int radius, float sigma_spatial, float sigma_color, int warmup_round, int test_round){
    ...
    auto runCase = [&](auto* h_src){
        ...
        //gpu版本
        RUNTIME_CHECK(cudaHostRegister(h_src_r101, src_size, cudaHostRegisterDefault));
        RUNTIME_CHECK(cudaHostRegister(h_dst, dst_size, cudaHostRegisterDefault));

        //预分配显存, 只需进行一次
        Type *d_src, *d_dst;
        cudaStream_t stream;
        RUNTIME_CHECK(cudaMalloc(&d_src, src_size));
        RUNTIME_CHECK(cudaMalloc(&d_dst, dst_size));
        ...
        //清理资源, 只需进行一次
        RUNTIME_CHECK(cudaHostUnregister(h_src_r101));
        RUNTIME_CHECK(cudaHostUnregister(h_dst));
        RUNTIME_CHECK(cudaFree(d_src));
        RUNTIME_CHECK(cudaFree(d_dst));
        ...
        printf("gpu( cuda ): Time: %8.3f ms | Throughput: %8.2f MP/s \n", time_ms_gpu, throughput_gpu);
...
void runAll(const std::string testerPath, int warmup_round, int test_round){
    ...
    cudaMemcpyToSymbol(spatial_lut_data, h_spatial_lut_data, sizeof(h_spatial_lut_data));
    ...
    cudaMemcpyToSymbol(color_lut_data, h_color_lut_data, sizeof(h_color_lut_data));
...
/*void runTester(const std::string& testerPath, int warmup_round, int test_round, bool only4K){
    ...
    for(const auto& entry : std::filesystem::recursive_directory_iterator(grayPath)){
        ...
        //gpu版本bF,统计数据并输出bin
        //注册pinned memory
        RUNTIME_CHECK(cudaHostRegister(src_gray_r101, src_size, cudaHostRegisterDefault));
        RUNTIME_CHECK(cudaHostRegister(dst_gray, dst_size, cudaHostRegisterDefault));
        ...
        //清理资源, 只需进行一次
        RUNTIME_CHECK(cudaHostUnregister(src_gray_r101));
        RUNTIME_CHECK(cudaHostUnregister(dst_gray));
        RUNTIME_CHECK(cudaFree(d_src));
        RUNTIME_CHECK(cudaFree(d_dst));
        RUNTIME_CHECK(cudaStreamDestroy(stream));
    ...
    for(const auto& entry : std::filesystem::recursive_directory_iterator(rgbPath)){
        ...
        //gpu版本bF
        //注册pinned memory
        cudaHostRegister(src_rgb_r101, src_size, cudaHostRegisterDefault);
        cudaHostRegister(dst_rgb, dst_size, cudaHostRegisterDefault);
        ...
        //清理资源, 只需进行一次
        RUNTIME_CHECK(cudaHostUnregister(src_rgb_r101));
        RUNTIME_CHECK(cudaHostUnregister(dst_rgb));
        RUNTIME_CHECK(cudaFree(d_src));
        RUNTIME_CHECK(cudaFree(d_dst));
        RUNTIME_CHECK(cudaStreamDestroy(stream));
